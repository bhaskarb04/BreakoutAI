...
void keypress(int key,int x,int y)
{
	...
	case(GLUT_KEY_LEFT):
		bg->move_paddle(true);
		key_pressed_current = GLUT_KEY_LEFT;
		break;

	case(GLUT_KEY_RIGHT):
		bg->move_paddle(false);
		key_pressed_current = GLUT_KEY_RIGHT;
		break;
	}
}
...
void display(void)
{
	...
	info_frame _frame;
	_frame.ballposx = ball->showctr().X();
	_frame.ballposy = ball->showctr().Y();
	_frame.mvballx = ball->showvel().X();
	_frame.mvbally = ball->showvel().Y();
	_frame.paddleposx = bg->paddle_pos().X();
	_frame.paddleposy = bg->paddle_pos().Y();
	_frame.keystroke = key_pressed_current;

	key_pressed_current = 0;
	if (!pause_flag)
		recorder::instance()->record(_frame);
